...
| 物模型 | 功能点 CRUD（属性/事件/命令），6宫格数据类型选择器，两栏动态表单 + 实时 JSON Schema 预览，克隆功能点 | ✅ |
...
本项目采用分级开发流程，根据需求规模选择对应流程：

| 等级 | 适用场景 | 流程 |
|------|---------|------|
| L1 小改动 | Bug 修复、样式调整 | 直接修复 → 验证 |
| L2 中等功能 | 新增弹窗、对接 API | 头脑风暴 → 开发 → 审查 → 验证 |
| L3 大功能 | 新模块、架构变更 | 头脑风暴 → PRD → UI/UX → 规划 → 并行开发 → 审查 → 验证 |

- **TDD 开发**: 后端服务和核心逻辑采用 TDD，详见 [TDD开发规划](docs/TDD/TDD开发规划.md)
