...
public class ThingModelController {
    ...
    /**
     * 发布物模型模板，状态变更为PUBLISHED
     *
     * @param id 模板ID
     * @return 发布后的模板
     */
    @PutMapping("/thing-model/templates/{id}/publish")
    @RequirePermission(module = "CATEGORY_MODEL", access = "RW")
    public Result<ThingModelTemplate> publishTemplate(@PathVariable Long id) {
        ThingModelTemplate template = templateService.publish(id);
